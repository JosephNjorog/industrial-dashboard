...
export async function getLock(machine) {
  const db = await getDb();
  const lock = await db.get(`SELECT * FROM locks WHERE machine = ?`, [machine]);
  
  if (lock && lock.expiresAt < Date.now()) {
    // Lock has expired, clean it up
    await db.run(`DELETE FROM locks WHERE machine = ?`, [machine]);
    return null;
  }
...
export async function acquireLock(machine, user, durationMs = 120000) {
  ...
  await db.run(
    `INSERT INTO locks (machine, user, expiresAt) VALUES (?, ?, ?)
...
export async function releaseLock(machine, user) {
  const db = await getDb();
  // Only release if the user matches (or admin force unlock, but we stick to user)
